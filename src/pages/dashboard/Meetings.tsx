...
import { Calendar, Clock, Users, Video, Phone, MapPin, FileText, Plus, Search, Filter, X, CheckCircle2, Circle } from 'lucide-react';
...
const statusColors = {
  ...
  cancelled: { bg: 'bg-gray-50', text: 'text-gray-700', border: 'border-gray-200', dot: 'bg-gray-500' },
};
...
export const MeetingsPage: React.FC = () => {
  ...
  const upcomingCount = mockMeetings.filter(m => m.status === 'scheduled').length;
  const completedCount = mockMeetings.filter(m => m.status === 'completed').length;
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Schedule and track all client meetings</p>
        </div>
        ...
        <Card className="p-4 rounded-xl hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 mt-1">{mockMeetings.length}</p>
            </div>
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-0.5">{meeting.client}</p>
                  </div>
                </div>
                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg ${statusColor.bg} ${statusColor.text} border ${statusColor.border}`}>
                  <div className={`w-2 h-2 rounded-full ${statusColor.dot}`} />
                  <span className="text-xs font-medium">{meeting.status.replace('_', ' ')}</span>
                </div>
              </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-14 h-14 rounded-xl ${statusColors[selectedMeeting.status].bg} flex items-center justify-center`}>
                  ...
                  <Badge className={`rounded-lg ${statusColors[selectedMeeting.status].bg} ${statusColors[selectedMeeting.status].text} border ${statusColors[selectedMeeting.status].border}`}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {selectedMeeting.location && (
                    <div className="p-3 bg-gray-50 rounded-xl">
                      ...
                      <p className="text-sm font-medium text-gray-900">{selectedMeeting.location}</p>
                    </div>
                  )}
                </div>
              </div>
